...
func main() {
	...
	cef.BrowserWindow.SetBrowserInit(func(event *cef.BrowserEvent, window cef.IBrowserWindow) {
		...
		if window.IsLCL() { // LCL window from
			...
			var (
				dlPicSave lcl.ISavePictureDialog
				dlPicOpen lcl.IOpenPictureDialog
			)
			...
			//window.Chromium().SetOnFileDialog(func(sender lcl.IObject, browser *cef.ICefBrowser, mode types.FileDialogMode, title, defaultFilePath string, acceptFilters *lcl.TStrings, callback *cef.ICefFileDialogCallback) bool {
			...
			callback.SetOnFileDialogDismissed(func(filePaths lcl.IStrings) {
			...
			ipc.On("showDialog", func(t int) {
				switch t {
				case 1:
					window.Chromium().Browser().RunFileDialog(types.FILE_DIALOG_OPEN_FOLDER, "打开文件夹", "", nil, callback)
				case 2:
					acceptFilters := lcl.NewStringList()
					acceptFilters.Add(".png")
					window.Chromium().Browser().RunFileDialog(types.FILE_DIALOG_SAVE, "保存图片", "", acceptFilters, callback)
				case 3:
					acceptFilters := lcl.NewStringList()
					acceptFilters.Add(".png")
					window.Chromium().Browser().RunFileDialog(types.FILE_DIALOG_OPEN, "打开图片", "", acceptFilters, callback)
				case 4:
					window.Chromium().Browser().RunFileDialog(types.FILE_DIALOG_SAVE, "打开", "", nil, callback)
				case 5:
					window.Chromium().Browser().RunFileDialog(types.FILE_DIALOG_OPEN, "保存", "", nil, callback)
				}
			})
		}
	})
