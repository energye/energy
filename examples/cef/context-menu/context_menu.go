...
func main() {
	...
	cef.BrowserWindow.SetBrowserInit(func(event *cef.BrowserEvent, window cef.IBrowserWindow) {
		var (
			menuId01           types.MenuId
			menuId02           types.MenuId
			menuId03           types.MenuId
			menuId0301         types.MenuId
			menuId0302         types.MenuId
			menuIdCheck        types.MenuId
			isMenuIdCheck      = true
			menuIdEnable       types.MenuId
			isMenuIdEnable     = true
			menuIdEnableCtl    types.MenuId
			menuIdRadio101     types.MenuId
			menuIdRadio102     types.MenuId
			menuIdRadio103     types.MenuId
			radioDefault1Check types.MenuId
			menuIdRadio201     types.MenuId
			menuIdRadio202     types.MenuId
			menuIdRadio203     types.MenuId
			radioDefault2Check types.MenuId
		)
		//右键弹出菜单
		event.SetOnBeforeContextMenu(func(sender lcl.IObject, browser *cef.ICefBrowser, frame *cef.ICefFrame, params *cef.ICefContextMenuParams, model *cef.ICefMenuModel, window cef.IBrowserWindow) bool {
			...
			if isMenuIdEnable {
				...
				model.SetColor(menuIdEnable, types.CEF_MENU_COLOR_TEXT, colors.NewCefARGB(255, 111, 12, 200))
			} else {
			...
			if !isMenuIdEnable {
				model.SetColor(menuIdEnableCtl, types.CEF_MENU_COLOR_TEXT, colors.NewCefARGB(255, 222, 111, 0))
			}
			model.AddSeparator()
			//radio 1组
		...
		//右键菜单项命令
		event.SetOnContextMenuCommand(func(sender lcl.IObject, browser *cef.ICefBrowser, frame *cef.ICefFrame, params *cef.ICefContextMenuParams, menuId types.MenuId, eventFlags uint32, window cef.IBrowserWindow) bool {
			fmt.Printf("params: %+v\n", params)
