...
func (m *MainWindowDemo) OnFormCreate(sender lcl.IObject) {
	...
	browserPanelBtn.SetOnClick(func(sender lcl.IObject) {
		...
		if chromiumBrowser == nil {
			...
			//禁止弹出新窗口
			chromiumBrowser.Chromium().SetOnOpenUrlFromTab(func(sender lcl.IObject, browser *cef.ICefBrowser, frame *cef.ICefFrame, targetUrl string, targetDisposition types.TCefWindowOpenDisposition, userGesture bool) bool {
			...
			// 以下是窗口关闭时 正确关闭浏览器的流程
			// windows的关闭示例
			chromiumBrowser.Chromium().SetOnClose(func(sender lcl.IObject, browser *cef.ICefBrowser, aAction *types.TCefCloseBrowserAction) {
				*aAction = types.CbaDelay
				lcl.QueueAsyncCall(func(id int) { //run in main thread
			...
			chromiumBrowser.Chromium().SetOnBeforeClose(func(sender lcl.IObject, browser *cef.ICefBrowser) {
				...
				lcl.QueueAsyncCall(func(id int) { // main thread run
