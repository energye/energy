	...
	*/
	...
	cef.BrowserWindow.SetBrowserInit(func(event *cef.BrowserEvent, window cef.IBrowserWindow) {
		...
		var buttonClickEvent = func(domRect cef.TCefRect) {
			...
			// 模拟鼠标双击事件
			//   左键点击按下1次
			chromium.SendMouseClickEvent(me, types.MBT_LEFT, false, 1)
			//   左键点击抬起1次
			chromium.SendMouseClickEvent(me, types.MBT_LEFT, true, 1)
		}
		// 模拟input输入
		...
		var textareaWheelEvent = func(domRect cef.TCefRect) {
			...
			// 鼠标事件
			me := &cef.TCefMouseEvent{
				Modifiers: types.EVENTFLAG_MIDDLE_MOUSE_BUTTON,
			}
			// 设置元素坐标，元素坐标相对于窗口，这里取元素中间位置
			me.X, me.Y = domXYCenter(domRect)
