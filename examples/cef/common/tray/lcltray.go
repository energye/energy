...
func LCLTray(browserWindow cef.IBrowserWindow) {
	...
	imageMenu.SetOnMeasureItem(func(sender lcl.IObject, aCanvas lcl.ICanvas, width, height *int32) {
		*height = 60
		*width = 270
	})
	imageMenuPng := lcl.NewIcon()
	imageMenuPng.LoadFromFSFile("resources/icon_red.ico")
	imageMenu.Bitmap().Canvas().DrawForGraphic(0, 0, imageMenuPng) //画上去
	imageMenuPng.Free()
	...
	tray.AddMenuItem("-", nil)
	var (
		radio1     lcl.IMenuItem
		radio2     lcl.IMenuItem
		radio3     lcl.IMenuItem
		groupIndex uint8 = 1
	)
	...
	tray.AddMenuItem("-", nil)
	var (
		showMenu lcl.IMenuItem
		wotMenu  lcl.IMenuItem
	)
