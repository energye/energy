...
func main() {
	...
	cef.BrowserWindow.SetBrowserInit(func(event *cef.BrowserEvent, window cef.IBrowserWindow) {
		...
		// 页面开始加载时 重置读取位置
		window.Chromium().SetOnLoadStart(func(sender lcl.IObject, browser *cef.ICefBrowser, frame *cef.ICefFrame, transitionType types.TCefTransitionType) {
			position = 0 // 重置位置 0
		})
		...
		var createFilter = func() *cef.ICefResponseFilter {
			...
			// 过滤器
			filter.Filter(func(dataIn uintptr, dataInSize uint32, dataInRead *uint32, dataOut uintptr, dataOutSize uint32, dataOutWritten *uint32) (status types.TCefResponseFilterStatus) {
				status = types.RESPONSE_FILTER_DONE
				...
				if dataIn != 0 && check == 1 { // 替换文本
					...
					*dataOutWritten = i
					if position < size {
						...
						status = types.RESPONSE_FILTER_NEED_MORE_DATA
					}
				}
				return
			})
