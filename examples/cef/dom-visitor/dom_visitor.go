...
func main() {
	...
	ipc.On("dom-visitor", func(channel callback.IChannel) {
		...
		// 发送渲染进程消息
		// data 不能为空, 且大于 0 个
		wi.Chromium().SendProcessMessageForJSONBytes("dom-visitor-test", types.PID_RENDER, []byte("test data"))
	})
	...
	// 监听渲染进程消息-在这里获取dom元素
	app.SetOnProcessMessageReceived(func(browser *cef.ICefBrowser, frame *cef.ICefFrame, sourceProcess types.CefProcessId, message *cef.ICefProcessMessage) bool {
