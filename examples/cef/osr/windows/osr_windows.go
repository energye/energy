...
func main() {
	...
	cefApp.SetOnScheduleMessagePumpWork(func(delayMS int64) {
		fmt.Println("delayMS", delayMS)
	})
...
func (m *WindowForm) bufferPanelEvent() {
	...
	m.bufferPanel.SetOnUTF8KeyPress(func(sender lcl.IObject, utf8key *types.TUTF8Char) {
		...
		if m.bufferPanel.Focused() {
			if utf8key.Len > 0 {
				...
				keyEvent.Modifiers = cef.GetCefKeyboardModifiers(types.WPARAM(asciiCode), 0)
				keyEvent.WindowsKeyCode = types.Int32(asciiCode)
				keyEvent.NativeKeyCode = 0
	...
	m.bufferPanel.(*cef.TBufferPanel).SetOnIMESetComposition(func(sender lcl.IObject, text string, underlines *cef.TCefCompositionUnderlineArray, replacementRange, selectionRange cef.TCefRange) {
