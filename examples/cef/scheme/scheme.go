...
func main() {
	...
	cefApp.SetOnRegCustomSchemes(func(registrar *cef.TCefSchemeRegistrarRef) {
		registrar.AddCustomScheme("hello", types.CEF_SCHEME_OPTION_STANDARD|types.CEF_SCHEME_OPTION_LOCAL)
	})
	cef.BrowserWindow.SetBrowserInit(func(event *cef.BrowserEvent, window cef.IBrowserWindow) {
		var idScheme types.MenuId
		var idClear types.MenuId
		var idURL types.MenuId
		// 在右键菜单实现这个示例
		...
		// 右键菜单命令
		event.SetOnContextMenuCommand(func(sender lcl.IObject, browser *cef.ICefBrowser, frame *cef.ICefFrame, params *cef.ICefContextMenuParams, commandId types.MenuId, eventFlags uint32, window cef.IBrowserWindow) bool {
				...
				factory.SetNew(func(browser *cef.ICefBrowser, frame *cef.ICefFrame, schemeName string, request *cef.ICefRequest) *cef.ICefResourceHandler {
					...
					handler.ProcessRequest(func(request *cef.ICefRequest, callback *cef.ICefCallback) bool {
						...
						if strings.Index(request.URL(), "hello-scheme") != 0 {
							filePath := filepath.Join(types.CurrentExecuteDir, "examples", "cef", "scheme", "resources", "hello-scheme.html")
