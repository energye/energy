...
func (f *TMainForm) OnButtonClick(sender lcl.IObject) {
	...
	// lcl.Screen.PixelsPerInch()

	rx := float64(lcl.Screen.PixelsPerInch()) / 96.0
	fmt.Println("lcl.Screen.PixelsPerInch(): ", lcl.Screen.PixelsPerInch(), ", rx: ", rx)
	//lcl.Printer.SetTitle("标题啊。。。。 ")

	lcl.Printer.BeginDoc()
	defer lcl.Printer.EndDoc()
...
func (f *TMainForm) OnCbbChange(sender lcl.IObject) {
	if f.CbbPrinters.ItemIndex() != -1 {
		lcl.Printer.SetPrinterIndex(f.CbbPrinters.ItemIndex())
	}
}
