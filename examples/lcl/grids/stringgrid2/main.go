...
func (f *TMainForm) FormCreate(sender lcl.IObject) {
	...
	f.dlgColor = lcl.NewColorDialog(f)

	f.pnl = lcl.NewPanel(f)
	f.pnl.SetParent(f)
	f.pnl.SetAlign(types.AlBottom)

	// 创建一个文本编辑框，不可视
	f.edit = lcl.NewEdit(f)
...
func (f *TMainForm) onEditChange(sender lcl.IObject) {
...
func (f *TMainForm) onGridButtonClick(sender lcl.IObject, aCol, aRow int32) {
...
func (f *TMainForm) onGridCheckboxToggled(sender lcl.IObject, aCol, aRow int32, aState types.TCheckBoxState) {
...
func (f *TMainForm) onGridDrawCell(sender lcl.IObject, aCol, aRow int32, aRect types.TRect, state types.TGridDrawState) {
...
func (f *TMainForm) onGridGetCheckboxState(sender lcl.IObject, ACol, ARow int32, value *types.TCheckBoxState) {
	if ARow > 0 && (ACol == 4) {
		*value = f.iData[ARow-1].CheckBox
	}
}

func (f *TMainForm) onGridGetCellHint(sender lcl.IObject, ACol, ARow int32, hintText *string) {
	switch ACol {
	case 0:
...
func (f *TMainForm) onGridGetEditMask(sender lcl.IObject, aCol, aRow int32, value *string) {
...
func (f *TMainForm) onGridSetEditText(sender lcl.IObject, aCol, aRow int32, value string) {
...
func (f *TMainForm) onGridValidateEntry(sender lcl.IObject, aCol, aRow int32, oldValue string, newValue *string) {
	//Constrain to '23:59:59'.
	//This only takes effect on leaving cell.
	s := []byte(*newValue)
	if len(s) > 0 && aRow > 0 && aCol == 1 {
		if s[0] > '2' {
			s[0] = '2'
		} else if s[1] > '3' {
			s[1] = '3'
