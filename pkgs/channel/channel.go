...
func IsUseNetIPC() bool {
	if tools.IsDarwin() || tools.IsLinux() {
		return false
	}
	goVersion := strings.Replace(runtime.Version(), "go", "", -1)
	vers := strings.Split(goVersion, ".")
	supoortWindows := conv.StrToInt32(vers[0]) >= 1 && conv.StrToInt32(vers[1]) >= 14 // Go 版本 >= 1.14 支持Windows
...
type IIPCContext interface {
	...
	ProcessId() cef.TCefProcessId // 返回 通道消息来源
	Message() IMessage            // 消息
	Free()                        //
}
...
type IPCContext struct {
	...
	processId   cef.TCefProcessId // ipc msg source, browser or render
...
func (m *IPCContext) ProcessId() cef.TCefProcessId {
	return m.processId
}
...
func (m *channel) write(messageType mt, channelId, toChannelId int64, data []byte) (n int, err error) {
	...
	var processId cef.TCefProcessId
	if m.channelType == CtServer {
		processId = cef.PID_BROWSER
	} else {
		processId = cef.PID_RENDERER
	}
	...
	for {
		...
		if size == headerLength {
			...
			m.handler(&IPCContext{
				...
				processId:   cef.TCefProcessId(proId),
