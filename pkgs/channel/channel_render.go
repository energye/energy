...
func NewRender(channelId int64, addresses ...string) IRenderChannel {
	...
	if useNetIPCChannel {
		...
		render.channel = &channel{conn: conn, channelId: channelId, ipcType: IPCT_NET, channelType: CtClient}
	} else {
		if len(addresses) > 0 {
			ipcSock = addresses[0]
		}
		...
		render.channel = &channel{conn: conn, channelId: channelId, ipcType: IPCT_UNIX, channelType: CtClient}
	}
	go render.receive()
