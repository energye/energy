...
func VariableBind(browser *cef.ICefBrowser, frame *cef.ICefFrame, bind cef.IProvisionalBindStorage) {
	/*
		通用类型变量和对象类型变量创建的回调函数
		该回调函数-在主进程和渲染进程创建时调用
	*/
	//ar integer cef.JSValue
	fmt.Println("GO变量和函数绑定回调 当前进程类型-ProcessType:", common.Args.ProcessType())
