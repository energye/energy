...
func (*domVisitor) New() *ICefDomVisitor {
	var result uintptr
	imports.Proc(internale_CefDomVisitorRef_Create).Call(uintptr(unsafe.Pointer(&result)))
	if result != 0 {
...
func (m *ICefDomVisitor) SetOnVisit(fn onVisit) {
	...
	imports.Proc(internale_CefDomVisitor_Visit).Call(m.Instance(), api.MakeEventDataPtr(fn))
}
